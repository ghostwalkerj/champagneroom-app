...
export type WalletMachineOptions = {
  saveStateCallback?: (wallet: WalletDocumentType) => void;
};
...
const createWalletMachine = ({ wallet }: { wallet: WalletDocumentType }) => {
...
export const createWalletMachineService = ({
  ...
  if (walletMachineOptions?.saveStateCallback) {
    walletService.onChange((context) => {
      walletMachineOptions.saveStateCallback &&
        walletMachineOptions.saveStateCallback(context.wallet);
    });
  }
