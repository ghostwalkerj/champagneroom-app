...
{#if $modalStore[0]}
	<div class="bg-surface-900 max-w-3xl rounded">
		<form class="grid sm:grid-cols-2" method="POST" use:enhance action={$modalStore[0].meta.action}>

            <input type="hidden" name="profileImage" value={$modalStore[0].meta.profileImage}>

            <img src={$modalStore[0].meta.profileImage} alt="profile" class="hidden sm:block rounded-l h-auto">

            <div class="p-4 flex flex-col gap-4 justify-between">
                <div class="flex flex-col gap-4">
                    <label class="label">
                        <span class="font-semibold">Name</span>
                        <input class="input variant-form-material" {...$constraints.name}
                         type="text"  placeholder="Enter name..." name="name" bind:value={$form.name}/>
                        {#if $errors.name}<span class="text-error">{$errors.name}</span>{/if}
                    </label>
                    <label class="label">
                        <span class="font-semibold">8 digit numeric PIN</span>
                        <input class="input variant-form-material" {...$constraints.pin}
                        type="number"  name="pin" bind:value={$form.pin} />
