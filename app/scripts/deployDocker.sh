...
ssh  "root@${SERVER_IP}" \
  "docker pull registry.digitalocean.com/champagneroom/pcall \
  && docker stop live-container \
  && docker rm live-container \
  && docker run --init -d --name live-container -p 3000:3000 registry.digitalocean.com/champagneroom/pcall \
